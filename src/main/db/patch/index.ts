...
const ensureBuiltinFreeModels = async (client: LibSqlClient): Promise<void> => {
  const now = Math.floor(Date.now() / 1000)
  const builtinModels = [
    {
  ...
  for (const model of builtinModels) {
    await client.execute({
      sql: `INSERT OR IGNORE INTO model_configs 
            (id, name, provider, model, api_key, base_url, max_tokens, 
             disable_temperature, thinking_parameter_mode, active, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        model.id,
        model.name,
        ...
        model.baseUrl,
        model.maxTokens,
        model.disableTemperature,
        model.thinkingParameterMode,
        model.active,
        now,
        now
      ]
...
export const runDatabasePatches = async (args: {
  ...
  await ensureBuiltinFreeModels(client)
}
