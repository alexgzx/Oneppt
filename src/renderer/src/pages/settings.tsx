...
import { ScanModelDialog } from '../components/settings/ScanModelDialog'
import {
...
export function SettingsPage(): React.JSX.Element {
  ...
  const [testResults, setTestResults] = useState<
    Record<string, { success: boolean; message: string; latency?: number } | null>
  >({})
  const [scanningProvider, setScanningProvider] = useState<string | null>(null)
  const [scanDialogOpen, setScanDialogOpen] = useState(false)
  const [scanDialogProvider, setScanDialogProvider] = useState<string>('opencode')
  ...
  const handleSaveModel = async (): Promise<void> => {
    ...
    if (!modelForm.apiKey.trim() && !isFreeModelProvider(modelForm.provider)) {
      warning(t('settings.fillApiKey'))
      return
    }
    if (!modelVerified && !isFreeModelProvider(modelForm.provider)) {
  ...
  const handleTestModel = async (config: ModelConfig): Promise<void> => {
    setTestingId(config.id)
    try {
      const result = await testModel(config.provider, config.model, config.baseUrl, config.apiKey)
      setTestResults((prev) => ({ ...prev, [config.id]: result }))
      if (result.success) {
        success(t('settings.testSuccess', { latency: result.latency }))
      } else {
        error(t('settings.testFailed', { message: result.message }))
      }
    } finally {
      setTestingId(null)
    }
  ...
  const handleScanModels = (provider: string): void => {
    setScanDialogProvider(provider)
    setScanDialogOpen(true)
  }

  const handleAddScannedModels = async (models: Array<{ id: string; name: string; isFree: boolean }>, provider: string): Promise<void> => {
    ...
    for (const model of models) {
      try {
        await upsertModelConfig({
          name: model.name,
          provider: provider as 'opencode' | 'kilo',
          model: model.id,
          apiKey: '',
          baseUrl,
          maxTokens: 4096,
        ...
        console.error(`Failed to add model ${model.name}:`, err)
      }
    }

    await fetchSettings()
    if (addedCount > 0) {
      success(t('settings.scanSuccess', { count: addedCount }), {
        description: t('settings.scanAdded', { total: models.length, added: addedCount })
      })
    }
  }
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      <Tabs defaultValue="general">
        ...
        <TabsContent value="model">
          <ModelSettingsTab
            ...
            onTest={(config) => void handleTestModel(config)}
            onScanModels={(provider) => void handleScanModels(provider)}
          />
        </TabsContent>
      ...
      <ScanModelDialog
        open={scanDialogOpen}
        provider={scanDialogProvider}
        existingConfigs={modelConfigs}
        t={t}
        onClose={() => setScanDialogOpen(false)}
        onAddModels={(models, provider) => void handleAddScannedModels(models, provider)}
      />
    </div>
  )
}
